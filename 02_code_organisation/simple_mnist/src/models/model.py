...
class MyAwesomeModel(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(in_channels=self.num_filter,
                               out_channels=self.num_filter,
                               kernel_size=3)
    ...
    def forward(self, x):
        ...
        # Conv1 + activation
        x = self.lrelu(self.conv1(x)) # [batch, num_filter, (28-(5-1)-1)+1, (28-(5-1)-1)+1] ~ [batch, num_filter, 24, 24]
        x = self.maxpool(x) # [batch, 16, 12, 12]
        # Conv2 -> activation
        x = self.lrelu(self.conv2(x)) # [batch, 16, (12-(3-1)-1)+1, (12-(3-1)-1)+1] ~ [batch, 16, 10, 10]
